        ...
        for image_index in range(batch_size):
            if result.nsfw_content_detected is None or not result.nsfw_content_detected[image_index]:
                ...
                if images_saved < num_images:
                    output_path = f"./generatedImages/result_{images_saved}.png"
                    result.images[image_index].save(output_path)
                    if image_callback:
                        image_callback(images_saved, output_path)
                    images_saved += 1
                    print(f"Image Generated in {output_path}")
...
def sd_main():
    ...
    if model_id not in model_to_image_size:
        print(
    ...
    model_dir = optimized_model_dir

    config.image_size = model_to_image_size.get(model_id, 512)
